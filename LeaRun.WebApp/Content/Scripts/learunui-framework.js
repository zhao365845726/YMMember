/**
 * jQuery LeaRunUI 4.1
 * YM.扬铭科技 Copyright © YangMing Technology 2014
 */
//样式
$(function () {
    readyIndex();
...
function delConfirm(url, parm, msg) {
    confirmDialog("YM.扬铭提示", msg, function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
...
function delConfig(url, parm, count) {
    ...
    confirmDialog("YM.扬铭提示", "注：您确定要删除 " + count + " 笔记录？", function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
function GetDeriveExcel(obj, fileName) {
    ...
    confirmDialog("YM.扬铭提示", "注：您确定要导出 " + data.length + " 笔记录？", function (r) {
        if (r) {
            var postData = {
